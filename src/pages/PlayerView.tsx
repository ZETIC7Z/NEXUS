...
export function RealPlayerView() {
  ...
  const resumeFromSourceId = usePlayerStore((s) => s.resumeFromSourceId);
  const storeSetResumeFromSourceId = usePlayerStore((s) => s.setResumeFromSourceId);
  ...
  const handleResumeScraping = useCallback(
    (startFromSourceId: string) => {
      // Set resume source first (in store so SourceSelectingView can also update it)
      storeSetResumeFromSourceId(startFromSourceId);
      // Then change status in next tick to ensure re-render
      setTimeout(() => {
        setStatus(playerStatus.SCRAPING);
      }, 0);
    },
    [setStatus, storeSetResumeFromSourceId],
  );
      ...
      {status === playerStatus.SCRAPING && scrapeMedia ? (
        ...
        isNativeApp() ? (
          <NativeScrapingPart
            ...
            onResult={() => {
              setScrapeNotFound();
              storeSetResumeFromSourceId(null);
            }}

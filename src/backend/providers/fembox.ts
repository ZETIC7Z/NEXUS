...
export async function scrapeFemboxMovie(
  ...
  let url = `https://goatapi.imreallydagoatt.workers.dev/api/febbox/movie/${tmdbId}?token=${encodeURIComponent(febboxKey)}`;
  if (turnstileToken) {
...
export async function scrapeFemboxTV(
  ...
  let url = `https://goatapi.imreallydagoatt.workers.dev/api/febbox/tv/${tmdbId}/${season}/${episode}?token=${encodeURIComponent(febboxKey)}`;
  if (turnstileToken) {
    url += `&turnstileToken=${encodeURIComponent(turnstileToken)}`;
  }
  const data = await fetchFemboxData(url);

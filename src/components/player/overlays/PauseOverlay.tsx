...
export function PauseOverlay() {
  ...
  useEffect(() => {
    ...
    const fetchLogoAndPoster = async () => {
      ...
      // Fetch backdrop/poster to show it during pause
      try {
        const detailsData = await getMediaDetails(meta.tmdbId, type, false);
        if (mounted) {
          if (isMobile) {
            // Mobile: display portrait poster
            if (detailsData?.poster_path) {
              const url = getMediaPoster(detailsData.poster_path);
              setPosterUrl(url || null);
            } else if (meta.poster) {
              setPosterUrl(meta.poster);
            }
          } else {
            // Desktop: display landscape backdrop
  ...
  }, [meta?.tmdbId, meta?.type, enableImageLogos, isMobile, meta?.poster]);
      ...
      >
        {isMobile && posterUrl && (
          <img
            src={posterUrl}
            alt={meta.title}
            className="mb-4 w-28 h-40 object-cover rounded-lg border border-white/10 shadow-lg drop-shadow-md pointer-events-none"
          />
        )}
        {logoUrl ? (
          <img
